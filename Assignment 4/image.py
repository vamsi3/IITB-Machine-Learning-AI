...
def read_image(filename):
	...
	with open(filename, 'rb') as f:
		...
		max_pixel_value = int(f.readline())
		if magic_number == 'P6':
...
def preprocess_image(img):
	...
	data = [tuple(pixel) for row in img for pixel in row] if type(img[0][0]) is list else [(pixel, pixel, pixel) for row in img for pixel in row]
...
def label_image(img, cluster_centroids):
	...
	def get_closest_centroid_idx(point):
		centroid_distances = map(lambda c: distance(point, c), cluster_centroids)
		return centroid_distances.index(min(centroid_distances))
	cluster_labels = [map(get_closest_centroid_idx, row) for row in img]
...
def write_image(filename, img):
	...
	magic_number = 'P6' if type(img[0][0]) is list else 'P5'
	height, width = len(img), len(img[0])
	max_pixel_value = max(map(max, (map(max, img)))) if magic_number == 'P6' else max(map(max, img))
...
def decompress_image(cluster_labels, cluster_centroids):
	...
	# img = None
	cluster_centroids = [map(int, c) for c in cluster_centroids]
	img = [map(cluster_centroids.__getitem__, row) for row in cluster_labels]
