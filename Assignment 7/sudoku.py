...
class SudokuSearchProblem:
    ...
    def getSuccessors(self, state):
        ...
        _, min_square = min((len(state[s]), s) for s in squares if len(state[s]) > 1)
        for digit in state[min_square]:
            next_state = assign(state.copy(), min_square, digit)
            if next_state:
                successors.append((next_state, (min_square, digit), 1))
