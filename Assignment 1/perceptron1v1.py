		...
		for iteration in range(self.max_iterations):
			...
			for i in range(len(trainingData)):
				...
				for j in range(len(self.legalLabels)):
					y_hat = self.legalLabels[j]
					if y_hat == y:
						continue
					idx = frozenset([y, y_hat])
					prediction = max(y, y_hat) if self.weights[idx] * trainingData[i] > 0 else min(y, y_hat)
					if prediction != y:
						if y > y_hat:
							self.weights[idx] += trainingData[i]
						else:
							self.weights[idx] -= trainingData[i]
