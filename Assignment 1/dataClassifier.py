...
def enhancedFeatureExtractorDigit(datum):
    ...
    # Feature 0:    Perimeter / sqrt(Area)
    # Feature 1:    min(distance from center among all points on perimeter)
    # Feature 2:    max(distance from center among all points on perimeter)
    ...
    for i in range(DATUM_WIDTH):
        for j in range(DATUM_HEIGHT):
            if getPixelVal(i, j):
                count += 1
                if (i <= 0 or getPixelVal(i-1, j) == 0) or (j <= 0 or getPixelVal(i, j-1) == 0) or \
                    (i >= DATUM_WIDTH-1 or getPixelVal(i+1, j) == 0) or (j >= DATUM_HEIGHT-1 or getPixelVal(i, j+1) == 0):
                    features[0] += 1
                    r = ((i - DATUM_WIDTH/2)**2 + (j - DATUM_HEIGHT/2)**2)**0.5
                    features[1] = min(features[1], r)
                    features[2] = max(features[2], r)
    features[0] /= count**0.5
